...
def load_clip_to_cpu(cfg):
    ...
    # Add MaPLe design details for multi-modal prompting
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # Add MaPLe compound prompts depth
        self.compound_prompts_depth = getattr(cfg.TRAINER.COOP, 'PROMPT_DEPTH', 1)
        assert self.compound_prompts_depth >= 1, "For MaPLe adaptation, PROMPT_DEPTH should be >= 1"
        ...
        print('TCP_MOD with MaPLe design: Multi-modal Prompt Learning')
        print(f'Initial context: "{prompt_prefix}"')
        print(f"Number of context words (tokens): {n_ctx}")

        self.ctx = nn.Parameter(ctx_vectors)  # to be optimized

        # Add MaPLe compound prompts for deeper layers
        self.compound_prompts_text = nn.ParameterList([nn.Parameter(torch.empty(n_ctx, 512))
                                                      for _ in range(self.compound_prompts_depth - 1)])
        for single_para in self.compound_prompts_text:
            nn.init.normal_(single_para, std=0.02)
        
        # Add projection layers for visual prompts
    ...
    def forward(self, text_features):
        ...
        # Get context vectors and expand for all classes
        ctx = self.ctx
        if ctx.dim() == 2:
        ...
        # Prepare visual prompts for MaPLe
        visual_deep_prompts = []
        for index, layer in enumerate(self.compound_prompt_projections):
            visual_deep_prompts.append(layer(self.compound_prompts_text[index]))
        
        # Project textual prompts from 512 to 768 for visual side
        shared_ctx = self.proj(self.ctx)
        
        return prompts, class_feature, shared_ctx, self.compound_prompts_text, visual_deep_prompts
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # Use MaPLe-style image encoder that accepts visual prompts
        self.image_encoder = clip_model.visual
        self.old_image_encoder = ImageEncoder(self.image_encoder)
    ...
    def forward(self, image, label=None):
        ...
        cls_token, patch_tokens = self.old_image_encoder(image.type(self.dtype))
        ...
        prompts, class_prompt, shared_ctx, compound_prompts_text, visual_deep_prompts = self.prompt_learner(atten_features)
        
        text_features = self.text_encoder(prompts, class_prompt, self.weight, tokenized_prompts.detach(), compound_prompts_text)
        
        image_features = self.image_encoder(image.type(self.dtype), shared_ctx, visual_deep_prompts)
...
class TCP_MOD_MAPLE(TrainerX):
    ...
    def build_model(self):
        ...
        name_to_update = ["prompt_learner", "cross_attn_text_img"]
        ...
        # NOTE: Build optimizer for the complete model to include all learnable parameters
        self.optim = build_optimizer(self.model, cfg.OPTIM)
        self.sched = build_lr_scheduler(self.optim, cfg.OPTIM)
        self.register_model("TCP_MOD_MaPLe", self.model, self.optim, self.sched)
