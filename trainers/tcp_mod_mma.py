...
class ImageEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, visual_adapter_func=None):
        # Extract the input tensor if it comes with adapter function
        if isinstance(x, list) and visual_adapter_func is not None:
            x = x[0]
        ...
        # Pass through transformer with adapter function if provided
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, class_feature, weight, tokenized_prompts, flag=False, text_adapter_func=None):
        ...
        if flag:
            ...
            if text_adapter_func is not None:
                # Use adapter-enabled transformer
                outputs = self.transformer.resblocks([x, class_feature, weight, counter, text_adapter_func])
            else:
...
class AdapterLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.use_adapters = getattr(cfg.TRAINER.TCP_MOD_MMA, 'USE_ADAPTERS', True)

        if self.use_adapters:
            # build multi-modal adapter for text encoder
            self.text_adapter_func = lambda x: self.return_text_adapter(index=x)
            text_layers = len(clip_model.transformer.resblocks)
            ...
            # build multi-modal adapter for vision encoder
            self.visual_adapter_func = lambda x: self.return_visual_adapter(index=x)
            visual_layers = len(clip_model.visual.transformer.resblocks)
            ...
            # shared adapter
            max_layers = max(text_layers, visual_layers)
            ...
            # No adapters - set to None
            self.text_adapter_func = None
            self.visual_adapter_func = None
    ...
    def return_text_adapter(self, index):
        if not self.use_adapters or self.text_adapter is None:
            return None, None, 0.0
        text_adapter = self.text_adapter[index] if index < len(self.text_adapter) else None
        shared_adapter = self.shared_adapter[index] if index < len(self.shared_adapter) else None
        return text_adapter, shared_adapter, self.adapter_scale

    def return_visual_adapter(self, index):
        if not self.use_adapters or self.visual_adapter is None:
            return None, None, 0.0
        visual_adapter = self.visual_adapter[index] if index < len(self.visual_adapter) else None
        shared_adapter = self.shared_adapter[index] if index < len(self.shared_adapter) else None
        return visual_adapter, shared_adapter, self.adapter_scale

    def _build_text_embedding(self, cfg, classnames, clip_model):
        dtype = clip_model.dtype
        text_ctx_init = getattr(cfg.TRAINER.TCP_MOD_MMA, 'TEXT_CTX_INIT', 'a photo of a')
    ...
    def forward(self):
        ...
        if self.use_adapters and self.text_adapter is not None and len(self.text_adapter) > 0 and self.text_adapter[0] is not None:
            token_embedding = self.text_adapter[0].down(embedding)
            if len(self.shared_adapter) > 0 and self.shared_adapter[0] is not None:
                token_embedding = self.shared_adapter[0](token_embedding)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # Add adapter learner if adapter settings are provided
        self.use_adapters = hasattr(cfg.TRAINER, 'TCP_MOD_MMA') and hasattr(cfg.TRAINER.TCP_MOD_MMA, 'ADAPTER_DIM')
        if self.use_adapters:
    ...
    def forward(self, image, label=None):
        # Get adapter functions if using adapters
        text_adapter_func = None
        visual_adapter_func = None
        if self.use_adapters:
        ...
        text_features = self.text_encoder(prompts, class_prompt, self.weight, tokenized_prompts.detach(), 
                                         flag=False, text_adapter_func=text_adapter_func) 
...
class TCP_MOD_MMA(TrainerX):
    ...
    def build_model(self):
        ...
        # Build optimizer and register models
        trainable_params = []
        trainable_params.extend(list(self.model.prompt_learner.parameters()))
        trainable_params.extend(list(self.model.cross_attn_text_img.parameters()))
        
        if self.model.use_adapters:
            trainable_params.extend(list(self.model.adapter_learner.parameters()))
        
        # Create a parameter group for optimization
        param_group = {'params': trainable_params}
        self.optim = build_optimizer([param_group], cfg.OPTIM)
